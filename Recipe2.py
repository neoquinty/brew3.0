...
##estimated values based on the recipe
	...
	def parseRecipe(self):
		print("Openingfile:"+self.filepath)
...
##self.hop_flav=hop.Hop(float(sub[3]),float(sub[2]),float(sub[1]),self.sys_vol,float(sub[4]))
	...
	def __estimated_time(self):
		...
		if self.mash_single_step_time==0:
			ramp_one=(self.mash_fitasi_temp-self.water_temp)*60
			step_one=self.mash_fitasi_time*60
			ramp_two=(self.mash_proteinrest_temp-self.mash_fitasi_temp)*60
			step_two=self.mash_fitasi_time*60
			ramp_three=(self.mash_betamilasi_temp-self.mash_proteinrest_temp)*60
			step_three=self.mash_betamilasi_time*60
			ramp_four=(self.mash_alphamilasi_temp-self.mash_betamilasi_temp)*60
			step_four=self.mash_alphamilasi_time*60
			ramp_out=(self.mashout_temp-self.mash_single_step_temp)*120

			result=result+ramp_one+step_one+ramp_two+step_two+ramp_three+step_three+ramp_four+step_four+ramp_out
			print("heat: "+str(ramp_one))
			print("fitasi: "+str(step_one))
			print("heat: "+str(ramp_two))
			print("protein rest: "+str(step_two))
			print("heat: "+str(ramp_three))
			print("betamilasi: "+str(step_three))
			print("heat: "+str(ramp_four))
			print("alphamilasi: "+str(step_four))
			print("heat: "+str(ramp_out))
		else:
			ramp_single=(self.mash_single_step_temp-self.water_temp)*60
			step_single=self.mash_single_step_time*60
			ramp_out=(self.mashout_temp-sel.mash_single_step_temp)*120
			result=result+ramp_single+step_single+ramp_out
		mashout_step=self.mashout_time*60
		mashout_pause=self.mash_pause*60
		sparge_ramp=(self.sparge_temp-self.water_temp)*60
		sparge_step=self.sparge_time*60
		sparge_pause=self.sparge_pause*60
		ramp_boil=(self.boil_temp-self.sparge_temp)*120
		boil_step=self.boil_time*60
		print("mashout: "+str(mashout_step))
		print("mashout_pause: "+str(mashout_pause))
		print("heat: "+str(sparge_ramp))
		print("sparge: "+str(sparge_step))
		print("sparge_pause: "+str(sparge_pause))
		print("ramp to boil: "+str(ramp_boil))
		print("boil: "+str(boil_step))
		result=result+mashout_step+mashout_pause+sparge_ramp+sparge_step+sparge_pause+ramp_boil+boil_step
			...
			if self.ibu>=int(l[1]) and self.ibu<=int(l[2]):
				if l[0]=="Porter" or l[0]=="Stout":
					if str(self.malt_chocolate.kg)=="0.0" and str(self.malt_black.kg)=="0.0" and str(self.malt_roasted_barley.kg)=="0.0":
						...
						style_str+=str(l[0])+","
##		ifl[0]=="Porter"orl[0]=="Stout":
##		ifself.malt_chocolate.kg!=0.0orself.malt_black.kg!=0.0orself.malt_roasted_barley!=0.0:
##	style_str+=str(l[0])+","
				else:
					if str(self.malt_chocolate.kg)=="0.0" and str(self.malt_black.kg)=="0.0" and str(self.malt_roasted_barley.kg)=="0.0":
						style_str+=str(l[0])+","
	...
	def print_recipe(self):
		...
		if self.malt_pale.kg!=0.0: print("PALEALE\t"+str(self.malt_pale.kg))
		...
		print("******************************************")
		if self.mash_single_step_time!=0:
			print("MASH\tSINGLESTEP\t"+str(self.mash_single_step_time)+"min@"+str(self.mash_single_step_temp)+"C")
		else:
		...
		print("MASHOUT\t\t"+str(self.mashout_time)+"min@"+str(self.mashout_temp)+"C")
		print("MASHPAUSE\t\t"+str(self.mash_pause)+"min")
		print("SPARGE\t\t\t"+str(self.sparge_time)+"min@"+str(self.sparge_temp)+"C")
		print("SPARGEPAUSE\t\t"+str(self.sparge_pause)+"min")
		print("BOIL\t\t\t"+str(self.boil_time)+"min@"+str(self.boil_temp)+"C")
		print("******************************************")
		print("******************************************")
		print("EstimatedResult@sys_eff="+str(self.sys_eff)+":")
		...
		print("")
		print("Overalltime: "+self.brew_time)
